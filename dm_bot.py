...
# ========== DATABASE ==========
def init_db():
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
...
def db_add_account(user_id, phone, session):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("INSERT INTO user_accounts (user_id, phone, session_string, added_date) VALUES (?, ?, ?, ?)",
...
def db_add_dm_history(user_id, target, message, count, sent):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("INSERT INTO dm_history (user_id, target, message, count, sent, date) VALUES (?, ?, ?, ?, ?, ?)",
...
def db_add_clone_payment(user_id, amount, utr):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("INSERT INTO clone_payments (user_id, amount, utr, date, status) VALUES (?, ?, ?, ?, 'pending')",
...
def db_add_clone(user_id, bot_token, bot_username):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("INSERT INTO bot_clones (user_id, bot_token, bot_username, created_date, status) VALUES (?, ?, ?, ?, 'stopped')",
...
def db_add_available_bot(bot_token, bot_username):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("INSERT INTO available_bots (bot_token, bot_username, status) VALUES (?, ?, 'available')",
...
def db_get_clone_by_token(user_id, bot_token):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("SELECT * FROM bot_clones WHERE user_id = ? AND bot_token = ?", (user_id, bot_token))
    clone = c.fetchone()
    conn.close()
    return clone
...
def db_update_clone_status(user_id, bot_token, status):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("UPDATE bot_clones SET status = ? WHERE user_id = ? AND bot_token = ?", (status, user_id, bot_token))
    conn.commit()
    conn.close()
...
def db_update_clone_env(user_id, bot_token, env_data):
    ...
    c.execute("UPDATE bot_clones SET env_data = ? WHERE user_id = ? AND bot_token = ?", (env_json, user_id, bot_token))
    conn.commit()
    conn.close()
...
def db_get_clone_env(user_id, bot_token):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("SELECT env_data FROM bot_clones WHERE user_id = ? AND bot_token = ?", (user_id, bot_token))
    result = c.fetchone()
    conn.close()
...
def db_add_pending_request(user_id, target_user_id):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("INSERT INTO pending_requests (user_id, target_user_id, request_date) VALUES (?, ?, ?)",
...
def db_add_redeem_code(code, reward_type, reward_value):
    conn = sqlite3.connect('dm_bot.db')
    c = conn.cursor()
    c.execute("INSERT INTO redeem_codes (code, reward_type, reward_value, created_date) VALUES (?, ?, ?, ?)",
