...
async def approve_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.get_bot().send_message(chat_id=user_id,
            text=f"🎉 *Premium Activated!*\n\n✅ Payment of ₹{PAYMENT_AMOUNT} approved!\n⭐ Now enjoy premium features!\n\n🤖 Clone your bot at @{os.getenv('CLONE_BOT_USERNAME')[1:]}",
            parse_mode="Markdown")
    except:
        pass
    await query.edit_message_text(f"✅ *Payment Approved!*\n\n👤 User: `{user_id}`\n⭐ Premium Activated!", parse_mode="Markdown")
...
async def approve_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in payments:
        ...
        try:
            await query.get_bot().send_message(chat_id=user_id, text=f"🎉 *Premium Activated!*\n\n✅ Payment approved!\n⭐ Enjoy premium!", parse_mode="Markdown")
        except:
            pass
    await query.edit_message_text(f"✅ *All Payments Approved!*\n\nTotal: {len(payments)} users upgraded!", parse_mode="Markdown")

async def add_clone_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("🤖 *Add Clone Bot*\n\nBot Token bhejein:\nFormat: `123456:ABC-DEF`\n\n📌 BotFather se token lo", parse_mode="Markdown")
...
async def add_bot_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ *Bot Added!*\n\n🔑 Token: `{bot_token}`\n📱 Username: {bot_username}\n\n📌 Available for cloning!", parse_mode="Markdown")
    return ConversationHandler.END

async def generate_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("🎫 *Generate Redeem Code*\n\n📌 Enter reward type:\n➡️ `premium` - Premium Access\n➡️ `100` - ₹100 Cash\n\nType reward type:", parse_mode="Markdown")
...
async def broadcast_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users:
        try:
            await update.get_bot().send_message(chat_id=user[0], text=f"📢 *Broadcast*\n\n{message}", parse_mode="Markdown")
            sent += 1
            await asyncio.sleep(0.1)
        except:
            pass
    await update.message.reply_text(f"✅ *Broadcast Sent!*\n\n📤 Sent to: {sent} users\n❌ Failed: {len(users) - sent}", parse_mode="Markdown")
    return ConversationHandler.END
...
async def login_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users[:20]:
        ...
        keyboard.append([InlineKeyboardButton(f"{username or user_id} {'⭐' if is_premium else '🆓'}", callback_data=f"admin_login_{user_id}")])
    keyboard.append([InlineKeyboardButton("🔙 Back", callback_data="admin_back")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text("🔐 *Select Account*\n\nClick to login as user:", reply_markup=reply_markup, parse_mode="Markdown")
...
async def login_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"🔐 *Login OTP*\n\n👤 Target User: `{target_user_id}`\n🔢 OTP: `{otp}`\n\n⬇️ Enter OTP to login:", parse_mode="Markdown")
...
async def verify_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if entered_otp == stored_otp:
        await update.message.reply_text(f"✅ *Logged in as User {target_user}*\n\n📌 Now you can use this account!\n🤖 Use @{os.getenv('DM_BOT_USERNAME')[1:]}", parse_mode="Markdown")
        return ConversationHandler.END
    else:
